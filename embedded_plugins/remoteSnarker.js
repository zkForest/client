  ...
  df.snarkHelper.snarkProverQueue.taskQueue = fastq(
    async function remoteSnarkerExecute(task, cb) {
      try {
        ...
        console.log(`replaced Q proved ${task.taskId}`);
        cb(null, res);
      } catch (e) {
  ...
  df.snarkHelper.snarkProverQueue.taskQueue = fastq(
    async function remoteSnarkerExecute(task, cb) {
      try {
        ...
        console.log(`remote snarker proved ${task.taskId}`);
        cb(null, res);
      } catch (e) {
...
//    input.placeholder = "https://snarker.orden.gg";
    input.placeholder = "https://my-app-2tzzx.ondigitalocean.app";
//    input.placeholder = "https://snark.1fa90.xyz";
    ...
    button.onclick = () => {
      ...
      firstTextDiv.innerText = "running";
      firstTextDiv.style.color = "green";
